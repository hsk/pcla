...
command1(Env,Js,newCommand(Com, Args),Sus) :-
  member(newcommands=Newcommands,Env),member(Com=Cmd,Newcommands),!,
         ...
         command1go(Sus1,Coms,Js_),!,commandM(Env,Js_,Sus)),
...
command1go((commandError(_, Exn, _),_),_,_) :- throw(Exn).
command1go((comReturn,Js),_,Js).
command1go((comAwait(_,_),Js),[],Js).
command1go((comAwait(Env,Cont),Js),[C|Coms],Js_) :- !,call(Cont,Env,Js,C,Sus1),command1go(Sus1,Coms,Js_).
